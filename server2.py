...
#     # JMeter behavior: if rank > n, take the last element (max)
...
#     data_sorted = sorted(data)
#     n = len(data)
#     if percentile == 90 or percentile == 95 or percentile == 99:
#         rank = math.floor((percentile / 100) * n)
#     else:
#         rank = math.ceil((percentile / 100) * n)
#     rank = max(1, min(rank, n))  # clamp
...
async function loadTotalTPS() {
    ...
    const resp = await fetch('/api/label_tps?window=' + window + (end ? ('&end=' + end) : ''));
    const data = await resp.json();
    const tz = $('#tzSelect').val();
...
def api_label_tps():
    ...
    # Get all labels in the time window
    label_rows = run_query("SELECT DISTINCT label FROM jmeter_samples WHERE timestamp BETWEEN ? AND ?", (start, end))
    labels = [r[0] for r in label_rows]

    # For each label, get TPS per second as float (count/1.0)
    label_tps = {}
    for label in labels:
        rows = run_query(
            "SELECT timestamp, COUNT(*) FROM jmeter_samples WHERE timestamp BETWEEN ? AND ? AND label=? GROUP BY timestamp ORDER BY timestamp ASC",
            (start, end, label)
        )
        ts_map = {r[0]: float(r[1]) for r in rows}
        tps = [round(ts_map.get(sec, 0.0), 2) for sec in range(start, end+1)]
        label_tps[label] = tps

    # Also return the timestamps for x-axis
